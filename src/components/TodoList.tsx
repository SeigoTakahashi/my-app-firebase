...
// TODOリストヘッダ
const tableHeads = ['title', 'status', 'note', ''];

// TODOリストタブ
const tableTabs = ['all', 'done', 'doing', 'note'];
...
const TodoList = () => {
    ...
    const filterTodos = (): { id: string; data: Todo }[] => {
        switch (filter) {
            case 'done':
            ...
            case 'doing':
                return todos.filter((t) => t.data.status !== 'done');
            case 'note':
                return todos.filter((t) => t.data.note);
            default:
                return todos;
        }
    };
    ...
    const sortTodos = (todo: { id: string; data: Todo }[]): { id: string; data: Todo }[] => {
        let result = todo;
        switch (sort[0]) {
            case 'status':
            ...
            case 'note':
                result.sort((a, b) => {
                    const noteA = a.data.note || '';
                    const noteB = b.data.note || '';
                    return noteA.localeCompare(noteB, 'ja');
                });
                if (sort[1] === 'desc') result.reverse();
                return result;
            default:
                ...
                >
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex shrink-0 flex-col gap-2 sm:flex-row">
                            {/* TODOリスト取得 */}
                            <Button variant="outlined" size="sm" onClick={() => setFilter("all")}>
                                view all
                            </Button>
                            {/* TODO登録ダイアログ表示 */}
                            <Button
                                    ...
                                    <tr key={todo.id}>
                                        ...
                                        <td className={classes}>
                                            ...
                                            <Tooltip content="Done">
                                                <IconButton variant="text" onClick={() => doneTodo(todo)}>
